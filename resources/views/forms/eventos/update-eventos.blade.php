...
<x-app-layout>
    ...
    <div class="flex w-full h-[800px]">
        <div class="bg-white border-[1px] border-solid border-gray-950 sm:rounded-lg m-auto h-[550px] w-[900px] p-6">
            <h1 class="text-center font-bold text-4xl mb-10">ATUALIZAR EVENTO</h1>
            <form method="POST" action="{{ route('eventos.update', $evento->id) }}" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <x-text-input id="titulo" class="block mt-1 w-full" type="text" name="titulo" :value="old('titulo', $evento->titulo)" required autofocus autocomplete="titulo" maxlength="30"/>
                ...
                <div class="mb-5">
                    ...
                    <x-text-input id="texto" class="block mt-1 w-full" type="text" name="texto" :value="old('texto', $evento->texto)" required autocomplete="texto" maxlength="255"/>
                    <x-input-error :messages="$errors->get('texto')" class="mt-2" />
                </div>

                <div class="mb-5">
                    <x-input-label for="hora_evento" :value="__('Horário do Evento')" />
                    <x-text-input id="hora_evento" name="hora_evento" type="datetime-local" class="mt-1 block w-full" required :value="old('hora_evento', $evento->hora_evento)" />
